...
export default function PolicyXRay() {
    ...
    const handleScan = async (idToScan?: string) => {
        const targetId = idToScan || policyId;
        if (!targetId) return;

        // Update state if triggered via feed click
        if (idToScan) setPolicyId(idToScan);
        ...
        try {
            const response = await fetch(`${API_URL}/xray/${targetId}`);
